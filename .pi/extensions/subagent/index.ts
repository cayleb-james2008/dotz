...
const TaskItem = Type.Object({
	...
	model: Type.Optional(Type.String({ description: "Model override (format: provider/model-id). Use a low-cost model from the injected list for task distribution." })),
});
...
const ChainItem = Type.Object({
	...
	cwd: Type.Optional(Type.String({ description: "Working directory for the agent process" })),
	model: Type.Optional(Type.String({ description: "Model override (format: provider/model-id). Use a low-cost model from the injected list." })),
});
...
const SubagentParams = Type.Object({
	...
	model: Type.Optional(Type.String({ description: "Model override for single mode (format: provider/model-id). Use a low-cost model from the injected list for task distribution." })),
});
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		description: [
			...
			"Model override: set `model` (format: provider/model-id) to run a subagent on a specific low-cost model.",
			"The system prompt includes a low-cost sub-model list — select from it for automatic task distribution.",
		].join(" "),
