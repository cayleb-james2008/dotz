...
0. RECALL & SCAFFOLD — `scout`: `memory_search` prior episodes/conventions. Pick an UNUSED mythological codename + kebab slug and the next episode N (highest Ep in $PANTHEON_HUB/README.md + 1, or 1 if none). Read your executive model from `bash`: `cat ~/.dotz/config.json` -> use its `executiveModel` verbatim (that is YOU; DOTZ_SUBAGENT_MODEL is the workers'). Then SCAFFOLD via `bash` (creates + pushes the public repo, tags episode/N, updates the hub):
   powershell -ExecutionPolicy Bypass -File $env:DOTZ_PANTHEON_HUB/scripts/new-episode.ps1 -Codename <codename> -Episode <N> -Model "<exec-model>" -Serve "Ollama Cloud" -Slug "<slug>" -Idea "<idea>" -DevPort 8090 -UpdateHub
...
9. SELF-IMPROVE & SCORE — `self-improvement-reviewer`: `rsi_baseline`/`rsi_compare` the gate, then spawn a scoring `subagent` (cwd `<codename>`) with a FIXED judge model (NOT your own; the same judge every episode) to rate `difficulty` (1-5) and `quality` (0-100: correctness, completeness, cleanliness, docs) with a one-line note, then via `bash`:
   powershell -ExecutionPolicy Bypass -File $env:DOTZ_PANTHEON_HUB/scripts/score-episode.ps1 -Codename <codename> -Difficulty <d> -Quality <q> -JudgeNote "<note>"
